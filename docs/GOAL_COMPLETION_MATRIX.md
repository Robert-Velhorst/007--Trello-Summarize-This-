...
| HAI connector | Implemented (live HAI authorization pending) | Per-user revocable capability URL, HMAC-only token storage, stable cursor, and explicit per-summary HAI approval. Live source creation remains under HAI owner authentication. |
...
| Windows installer | Implemented and acceptance-tested | Single `.exe`, bundled Node 22 backend, generated ACL-protected secrets, loopback local database, Start menu launchers, ngrok launcher, and verified uninstall. No Node, Docker, or admin rights required. |
...
| GitHub Pages deployment | Implemented in repository | Allowlisted 24-file static build and Pages workflow. Live deployment from `main` remains a release action. |
