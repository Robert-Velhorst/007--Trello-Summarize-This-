...
assert.match(cloudLauncherScriptText, /storage\.users/);
assert.match(installerInstallText, /Configure ngrok domain\.lnk/);
assert.match(installerInstallText, /REGISTRATION_MODE\s*=\s*"single-user"/);
...
const indexEntryText = fs.readFileSync(path.join(__dirname, "index.js"), "utf8");
assert.doesNotMatch(indexEntryText, /Inherited file content will not be shown/);
assert.match(indexEntryText, /require\("\.\/local-dev-server"\)/);
...
assert.equal(adminConfig.connectorUrl, "https://powerup.example.com/app/connector.html?v=20260814.3");
...
assert.ok(adminValuesText.includes("iframe Connector URL: https://powerup.example.com/app/connector.html?v=20260814.3"));
...
assert.ok(adminAutofillScript.includes("https://powerup.example.com/app/connector.html?v=20260814.3"));
