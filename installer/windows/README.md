...
The installed payload includes the 24 allowlisted static runtime files, standalone backend, local `update.json`, Trello setup assistant, cloud launcher, and uninstall helper. The backend prefers `127.0.0.1:18787`, selects a safe loopback fallback when that port is occupied, uses a private local JSON store by default, and starts only one process.
...
`Configure ngrok domain` validates and saves a reserved HTTPS domain in the per-user installation. `Share Backend with ngrok` starts the backend and then runs the user's installed ngrok client with that domain; the setting survives installer upgrades. When no domain is saved, the launcher clearly warns before allowing ngrok to select the account's default development domain. No tunnel starts during installation or ordinary app launch. Keep the ngrok window open while a hosted Trello Power-Up or HAI source uses the HTTPS endpoint. Browser API calls to ngrok's free development domains automatically include ngrok's documented interstitial-bypass header; other backend hosts never receive that header.
