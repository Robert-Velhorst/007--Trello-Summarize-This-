...
`Share Backend with ngrok` starts the backend and then runs the user's installed ngrok client. No tunnel starts during installation or ordinary app launch. Keep the ngrok window open while a hosted Trello Power-Up or HAI source uses the HTTPS endpoint.
